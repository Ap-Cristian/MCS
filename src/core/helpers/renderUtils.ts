...
export function generateRenderPipeline(
  ...
  if (arg.device) {
    ...
    const stride: number = Float32Array.BYTES_PER_ELEMENT * 4;
    // 3 floats * 4 bytes (pos) 
    // + 3 floats * 4 bytes (norm)
    // + 2 floats * 4 bytes (uv)
    // check obj file
    //
    ...
    return arg.device.createRenderPipeline({
      ...
      primitive: {
        ...
        cullMode: "none",
        stripIndexFormat: arg.stripIndexFormat,
...
export function IVertexArrToF32Arr(
  ...
  const posLength = array[0].pos.length; 
  const normLength = array[0].norm?.length ?? 0; 
  const uvLength = array[0].uv?.length ?? 0; 

  const arrayLength =
    (posLength + normLength + uvLength) * array.length;
...
export function IVertexToNumber(array: IVertex[]) {
  let result: number[] = [];

  array.forEach((vertex) => {
    result = result.concat(vertex.pos);
    result = vertex.norm ? result.concat(vertex.norm) : result;
    result = vertex.uv ? result.concat(vertex.uv) : result;
  });
